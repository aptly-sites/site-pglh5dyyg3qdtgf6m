...
const ALLOWED_CITIES = new Set(["waco", "woodway", "hewitt", "robinson", "china spring", "bellmead", "lacy lakeview"]);

type LeadPayload = Record<string, unknown>;
type AptlyField = { key?: string; uuid?: string; label?: string; name?: string; type?: string; archived?: boolean };
...
function fieldName(field: AptlyField) { return normalized(field.label || field.name); }
function fieldKey(field: AptlyField) { return field.key || field.uuid || ""; }

function findField(fields: AptlyField[], names: string[], types?: string[]) {
...
async function aptlyFetch(path: string, token: string, init?: RequestInit) {
  ...
  if (!response.ok) throw new Error(`Aptly ${path} returned ${response.status}: ${JSON.stringify(data)?.slice(0, 500)}`);
  return data;
}

async function verifyPropertyAddress(input: string) {
  ...
  if (!match || !components?.zip || components?.state !== "TX" || !ALLOWED_CITIES.has(normalized(components.city))) return null;
...
export async function POST(request: Request) {
  const token = process.env.APTLY_API_TOKEN;
  const boardId = process.env.APTLY_OWNER_LEADS_BOARD_ID;
  if (!token || !boardId) {
    console.error("Owner lead integration is missing its hosted Aptly configuration.");
    return Response.json({ message: "We couldn't send your request right now. Please call (254) 400-2863." }, { status: 503 });
  }

  let body: LeadPayload;
  try { body = await request.json(); }
  catch { return Response.json({ message: "Please review the form and try again." }, { status: 400 }); }
  ...
  if (!name || !email || !phone || !addressInput || !/^\S+@\S+\.\S+$/.test(email)) {
    return Response.json({ message: "Please complete your name, contact details, and full property address." }, { status: 400 });
  }

  try {
    const verified = await verifyPropertyAddress(addressInput);
    if (!verified) return Response.json({ message: "Please enter a valid property address in JR Grace Realty's service area." }, { status: 422 });
    ...
    const schemaFields: AptlyField[] = Array.isArray(schemaResponse) ? schemaResponse : schemaResponse?.data || [];
    const config = configurationResponse?.data || configurationResponse || {};
    const configFields: AptlyField[] = Array.isArray(config.fields) ? config.fields : [];
    const fields = [...schemaFields, ...configFields].filter((field, index, all) =>
      index === all.findIndex((candidate) => fieldKey(candidate) === fieldKey(field)));
    ...
    const optionalMappings: Array<[string[], unknown]> = [
      [["Property Type"], clean(body.propertyType, 80)], [["Bedrooms"], clean(body.bedrooms, 20)],
      [["Bathrooms"], clean(body.bathrooms, 20)], [["Owner Goal", "Goal"], clean(body.goal, 120)],
      [["Desired Management Service", "Desired Service", "Service Requested"], clean(body.goal, 120)],
      [["Message", "Notes", "Property Notes"], clean(body.message, 1200)], [["Email"], email], [["Phone", "Mobile Phone"], phone],
    ...
    const newLeadsWorkflow = workflows.find((workflow: Record<string, unknown>) =>
    ...
    return Response.json({ message: "We couldn't send your request right now. Please call (254) 400-2863." }, { status: 502 });
  }
}
